...
public class ProductosProxyController {
    ...
    public ResponseEntity<?> proxyProductos(HttpServletRequest request,
        ...
        String targetUrl = "http://localhost:8083/api/productos" + originalPath;
        HttpMethod method = HttpMethod.valueOf(request.getMethod());

        // Validación para POST, PUT y DELETE: solo admin puede realizar estas acciones
        if (method == HttpMethod.POST || method == HttpMethod.PUT || method == HttpMethod.DELETE) {
            ...
            if (!"admin".equalsIgnoreCase(rol)) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body("{\"error\": \"Solo admin puede modificar, crear o eliminar productos\"}");
            }
        }
